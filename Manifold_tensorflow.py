...
args = parser.parse_args(['--dataset', 'CIFAR10', '--ALTER', 'True', '--M', '500', '--epochs', '40', '--alter_steps', '1000',                           '--KNN', 'True', '--MTC', 'True', '--beta', '0.1', '--lambd', '10.0', '--gamma', '1.5',                           '--epsilon', '0.2', '--numlayers', '2', '--code_size', '768',                         '--code_size2', '192', '--k', '30', '--lr', '0.001',                          '--save_autoencoder_path', 'saved_weights/ae', '--optimized_SVD', 'True'])
#                          '--pretrained_autoencoder_path', 'saved_weights/ae', \
print(args)
...
if args.pretrained_autoencoder_path is None:
    ...
    if args.ALTER is True:
        ...
        for (x, _) in train_loader:
            TRAIN.append(np.array(x.view(batch_size, -1)))
        for (x, _) in train_z_loader:
            Z_TRAIN.append(np.array(x.view(batch_size, -1)))

        x_train = np.reshape(np.array(TRAIN), (-1,dimensionality))
        grad_x_train = np.reshape(np.array(Z_TRAIN), (-1,dimensionality))
        Autoencoder, Tangent = RRJ2(x_train, grad_x_train, k = args.k, code_size1=args.code_size, code_size2=args.code_size2,                             batch_size=args.batch_size,gamma=args.gamma,epsilon=args.epsilon, Lambda=args.lambd,                             iter_num=args.epochs, steps_number=args.alter_steps, learning_rate=args.lr, new_curvature = True)
        [cur_W1, cur_b1, cur_W2, cur_b2, cur_W3, cur_b3, cur_W4, cur_b4] = Autoencoder
        model.Copy(np.transpose(cur_W1), np.transpose(cur_b1),                    np.transpose(cur_W2), np.transpose(cur_b2),                    np.transpose(cur_W3), np.transpose(cur_b3),                    np.transpose(cur_W4), np.transpose(cur_b4))
        if args.save_autoencoder_path:
            torch.save(model.state_dict(), args.save_autoencoder_path)
...
if args.MTC is True:
    ...
    if args.ALTER:
        X, U, N = calculate_singular_vectors_B(model, train_loader, args.k, batch_size)
    else:
        X, U, N = calculate_singular_vectors_B(model, train_loader, args.dM, batch_size)
    ...
    for epoch in range(args.MTC_epochs):
        ...
        for (x, y) in train_loader:
            ...
            x.requires_grad_(True)
            y = y.cuda()
            x_prime = X[rnd].view(batch_size, -1).cuda()
            x_prime.requires_grad_(True)
            u_prime = U[rnd].cuda()
            pred = MTC_model(x)
            pred_prime = MTC_model(x_prime)
            loss, loss1 = MTC_loss(pred, y, pred_prime, x_prime, u_prime, args.beta, args.batch_size)
            x.requires_grad_(False)
...
if args.KNN:
    ...
    for test_image in test_images:
        ...
        for h in ks:
            pred = Counter(top_n_labels[:h]).most_common(1)[0][0]
    ...
    accuracies = {h: round((v/i) * 100, 2) for h, v in total_correct.items()}

    for h in ks:
        writer.add_scalar('K_acc', accuracies[h], h)
        with open('results_CAEH_tied_0.txt', 'a') as f:
            if args.ALTER is True:
                f.write("{}\t{}\t{}\t{}\t{}\t{}\t{}\t{}\n".format(args.lr,
                                                                  args.lambd, args.gamma, args.code_size, args.code_size2, args.epsilon, h, accuracies[h]))
            else:
